...
import { getReceivedOffers, acceptOffer, rejectOffer, PriceOffer } from '@/utils/offers';
...
export default function ProfessionalOffersPage() {
  ...
  const handleAcceptOffer = async (offerId: string) => {
    if (!confirm('Accept this offer? A booking will be automatically created.')) return;

    try {
      setProcessingId(offerId);
      await acceptOffer(offerId);
      alert('Offer accepted! Booking has been created.');
      await fetchOffers();
    } catch (err: any) {
              ...
              >
                  ...
                  <div className="flex-1">
                    ...
                    {offer.user_details && (
                      <div className="flex items-center gap-2 text-sm text-gray-600 mb-3">
                        ...
                        <span>{offer.user_details.full_name}</span>
                        {offer.user_details.phone && (
                          <span className="text-gray-400">• {offer.user_details.phone}</span>
                        )}
                      </div>
                    )}
                ...
                {offer.status === 'pending' && (
                  <div className="flex gap-3 pt-4 border-t border-gray-200">
                    ...
                    <button
                      onClick={() => openRejectModal(offer._id)}
                      disabled={processingId === offer._id}
